...
function applyFilter() {
  ...
  const newPapers = allPapers.filter(p => p.is_new);
  summary.textContent = "共 " + allPapers.length + " 篇，新增 " + newPapers.length + " 篇，收藏 " + favorites.size + " 篇，当前展示 " + papers.length + " 篇（按 " + (dateMode === "crawled_date" ? "抓取日期" : "发表日期") + " 筛选）";
}
...
async function init() {
  ...
  const newCount = allPapers.filter(p => p.is_new).length;
  document.getElementById("metaText").textContent =
    "收录 " + (payload.count || allPapers.length) + " 篇 | 新增 " + newCount + " 篇 | 抓取区间 " + (defaultMin || "-") + " ~ " + (defaultMax || "-");
